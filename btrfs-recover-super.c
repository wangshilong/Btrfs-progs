...
static int scan_fs_devices(int fd, const char *path,
			  struct btrfs_fs_devices **fs_devices)
{
	u64 total_devs;
	int ret;
	...
	if (total_devs != 1) {
		ret = btrfs_scan_for_fsid(*fs_devices, total_devs, 0);
		if (ret)
			return ret;
	}
	return 0;
...
static struct super_block_record *recover_get_good_super(struct btrfs_recover_superblock *recover)
{
	struct super_block_record *record;
	record = list_entry(recover->good_supers.next,
				struct super_block_record, list);
...
int main(int argc, char **argv)
{
	...
	ret = scan_fs_devices(fd, dname, &recover.fs_devices);
	close(fd);
	if (ret)
		goto no_recover;
